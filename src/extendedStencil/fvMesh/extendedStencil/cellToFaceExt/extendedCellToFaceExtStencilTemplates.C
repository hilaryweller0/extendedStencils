...
void Foam::extendedCellToFaceExtStencil::collectData
(
...
)
{
    ...
    map.distribute
    (
        fld.mesh().globalData().globalTransforms(),
        flatFld,
        top
    );
    ...
    forAll(stencil, faceI)
    {
        ...
        const labelList& transformedCompactCells = transformedElements[faceI];

        stencilFld[faceI].setSize(compactCells.size()+transformedCompactCells.size());

        label nCompact = 0;
        forAll(compactCells, i)
        {
            label index = compactCells[i];
            stencilFld[faceI][nCompact++] =
            (
                index >= 0
              ? flatFld[index]
              : pTraits<Type>::zero
        ...
        {
            label index = transformedCompactCells[i];
            stencilFld[faceI][nCompact++] =
            (
                index >= 0
              ? flatFld[index]
...
Foam::extendedCellToFaceExtStencil::weightedSum
(
    const mapDistribute& map,
    const labelListList& stencil,
    const labelListList& transformedElements,
...
)
{
    ...
    List<List<Type> > stencilFld;
    collectData(map, stencil, transformedElements, fld, stencilFld, top);
