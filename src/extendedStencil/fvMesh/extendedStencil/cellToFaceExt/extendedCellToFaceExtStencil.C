...
)
{
    ...
    forAll(stencil, i)
    {
        ...
        if (sCells.size() > 0)
        {
            ...
            minSizeUntransformedElements = min
            (
                minSizeUntransformedElements,
                sCells.size()
            );
            maxSizeUntransformedElements = max
            (
                maxSizeUntransformedElements,
                sCells.size()
            );
        }
    }
    ...
    forAll(transformedElements, i)
    {
        ...
        if (sCells.size() > 0)
        {
            ...
            minSizeTransformedElements = min
            (
                minSizeTransformedElements,
                sCells.size()
            );
            maxSizeTransformedElements = max
            (
                minSizeTransformedElements,
                sCells.size()
            );
...
Foam::extendedCellToFaceExtStencil::extendedCellToFaceExtStencil
(
    const polyMesh& mesh
)
:
