...
int main(int argc, char *argv[])
{
    ...
    const globalIndexAndTransform& git = mesh.globalData().globalTransforms();
    Pout<< "null:" << git.nullTransformIndex() << endl;

    // Calculate per cell the (face) connected cells (in global numbering)
    CPCCellToCellExtStencil globalCellCells(mesh);

    {    
        const labelListList& untrafoElems =
            globalCellCells.untransformedElements();
        const List<labelPairList>& trafoElems =
            globalCellCells.transformedElements();
        forAll(untrafoElems, cellI)
        {
            Pout<< "cell:" << cellI << " at:" << mesh.C()[cellI] << endl;
            if (untrafoElems[cellI].size())
            {
                const labelList& elems = untrafoElems[cellI];
                Pout<< "untransformed:" << endl;
                forAll(elems, i)
                {
                    Pout<< "    " << elems[i] << endl;
                }
            ...
            if (trafoElems[cellI].size())
            {
                const labelPairList& elems = trafoElems[cellI];
                Pout<< "transformed:" << endl;
                forAll(elems, i)
                {
                    ...
                    Pout<< "    proc:" << neiProc << endl;
                    Pout<< "    index:" << neiIndex << endl;
                    Pout<< "    transform:" << neiTransform << endl;
                }
            }
        }
        Pout<< endl;
    ...
    CPCCellToFaceExtStencil globalFaceCells(mesh);

    {
        const labelListList& untrafoElems =
            globalFaceCells.untransformedElements();
        const List<labelPairList>& trafoElems =
            globalFaceCells.transformedElements();

        forAll(untrafoElems, faceI)
        {
            Pout<< "face:" << faceI << " at:" << mesh.faceCentres()[faceI]
                << endl;
            if (untrafoElems[faceI].size())
            {
                Pout<< "untransformed:" << endl;
                const labelList& elems = untrafoElems[faceI];
                forAll(elems, i)
                {
                    Pout<< "    " << elems[i] << endl;
                }
            ...
            if (trafoElems[faceI].size())
            {
                Pout<< "transformed:" << endl;
                const labelPairList& elems = trafoElems[faceI];
                forAll(elems, i)
                {
                    const labelPair& elem = elems[i];
